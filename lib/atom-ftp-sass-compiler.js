...
export default {
    ...
    compile() {
        ...
        if (setup) {
            // Compile file to ftp
            const pathToLib = __dirname + '/ftp-sass-compiler/lib/';
            const pathToFiles = __dirname + '/ftp-sass-compiler';
            // Cleanup class
            const Cleanup = require(pathToLib + 'cleanup.js');
            let clean = new Cleanup(pathToFiles);

            // Convert class
            const Convert = require(pathToLib + 'convert.js');
            let sass = new Convert(pathToFiles);
            ...
            function ftpConfig() {
                ...
                if (sftpConfig) {
                    ...
                    const Sftp = require(pathToLib + 'sftp.js');
                    return new Sftp(pathToFiles);
                } else {
                    console.log("Using ftp");
                    // Ftp class
                    const Ftp = require(pathToLib + 'ftp.js');
                    return new Ftp(pathToFiles);
                }
